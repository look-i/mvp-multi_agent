...
async def create_message(req: MessageCreateRequest):
    ...
    try:
        # 1. 将用户的消息存入数据库
        user_message_res = supabase.table('messages').insert({
        ...
        # 2. 从数据库获取完整的历史消息
        history_res = supabase.table('messages').select('*').eq('conversation_id', req.conversation_id).order('created_at', desc=False).execute()
        ...
        # 7. 将智能体的回复存入数据库
        agent_message_res = supabase.table('messages').insert({
            'conversation_id': req.conversation_id,
